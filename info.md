...
- 제약들간의 우선순위
- 상황에 따라서 뷰들의 크기가 유동적으로 변하는 경우가 있는데, 이 때 제약들간에 충돌이 일어날 수 있다. 이럴때에는 제약의 우선순위를 매겨 충돌을 해결할 수 있다.

## UICollectionViewLayout - prepare
- 레이아웃 관련 연산이 일어날 때 마다 가장먼저 호출.
- 셀의 위치/크기 등을 계산하기 위한 사전처리를 할 수 있다.
...
- 비동기를 동기화 된 것처럼 작성 가능.

2. 일관성이 없는 비동기 코드를 해결
- 하나의 비동기 코드로 개발이 가능.

3. 확장 불가능한 아키텍처 패턴을 해결
- 일관성 없는 비동기 코드를 작성하게 되어 서로 다르게 구현한 로직을 조합하거나 확장하기에 어려운 부분을 해결한다.
- Rx로 일관된 코드를 작성하면서 아키텍처의 확장이 가능하다.

4. 콜백지옥에서 벗어나기 좋음
- UI 이벤트, 네트워크 처리 및 데이터 갱신 등에서 콜백 지옥이나 이벤트 등록 후 추적, 스레드를 넘나들면서 작업하던 상황에서 RxSwift를 이용하여 가독성을 높이고 스레드를 쉽게 넘나들며 콜백 지옥을 탈출
